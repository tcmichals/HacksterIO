// 4-Way Interface Protocol for BLHeli ESC configuration
#ifndef FOURWAY_HPP
#define FOURWAY_HPP
...
constexpr uint8_t FOURWAY_PC_SYNC = 0x2F;  // PC → FC request
constexpr uint8_t FOURWAY_FC_SYNC = 0x2E;  // FC → PC response

// 4-Way commands
enum FourWayCmd : uint8_t {
...
constexpr int FOURWAY_MAX_PARAMS = 256;

// 4-Way message structure
struct fourway_msg_t {
    uint8_t  sync;
    uint8_t  cmd;
    uint16_t address;
...
// Returns when CMD_INTERFACE_EXIT is received or timeout
void fourway_loop();
