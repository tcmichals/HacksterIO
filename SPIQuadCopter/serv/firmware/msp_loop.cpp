...
#ifdef TEST_HOST
inline void dbg_set(uint8_t val) { (void)val; }
#else
...
int MspStateMachine::process_char(uint8_t ch, uint8_t* tx_buf) {
    ...
    case MspState::WAIT_DOLLAR:
        if (ch == '$') {
            ...
            dbg_set(10);  // Noise in WAIT_DOLLAR state
        break;
...
// DSHOT is sent only when explicitly commanded via MSP_SET_MOTOR
void msp_loop() {
    MspStateMachine sm;
        ...
        if (ch & 0x0100) {
            ...
            if (tx_len > 0) {
                ...
                if (enter_passthrough_mode) {
                    ...
                    fourway_loop();  // Returns when CMD_INTERFACE_EXIT received
                }
            }
        }
    }
