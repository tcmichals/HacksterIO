...
// Debug state values - toggle count patterns for single-bit GPIO
// Each value = number of toggle pulses to output (visible on scope)
#define DBG_RESET      1
#define DBG_RUNNING    2
#define DBG_LOOP       3
#define DBG_RX         4
#define DBG_MSP_START  5
#define DBG_MSP_DONE   6
#define DBG_TX_DONE    7
#define DBG_ERROR      8
...
public:
    MspStateMachine() : state_(MspState::WAIT_DOLLAR), rx_idx_(0), payload_len_(0), payload_remaining_(0) {}
    ...
    // Reset state machine
    void reset() { state_ = MspState::WAIT_DOLLAR; rx_idx_ = 0; payload_len_ = 0; payload_remaining_ = 0; }
    
private:
    uint8_t rx_buf_[MSP_MAX_PAYLOAD + 16];
    MspState state_;
    int rx_idx_;
    int payload_len_;      // Payload length from header
    int payload_remaining_; // Bytes remaining to read in payload
};
