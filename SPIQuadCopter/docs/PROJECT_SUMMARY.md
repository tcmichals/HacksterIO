...
The Tang Nano 9K Quadcopter FPGA project implements a Flight Controller (FC) bridge on a Gowin GW1N-9C FPGA. The design features a **simplified dual Wishbone bus architecture** with:

- **SERV RISC-V CPU**: Handles protocol processing (MSP, 4-Way Interface, ESC configuration)
- **SPI-to-Wishbone Bridge**: Provides external flight controller direct access to DSHOT motors

The system operates at 72 MHz with clean separation: SPI controls motors, SERV handles protocols.
...
```
  ...
  SPI (from FC) ──► │  SPI Slave ─► wb_mux_6 ──┬─► LED (0x100)   │
                    ...
                    │                          └─► Mux Mirror    │
                    │                                             │
  USB UART ◄──────► │  SERV CPU ──► wb_mux_4 ──┬─► Debug GPIO   │
                    │                          ├─► Mux Reg       │
                    │                          ├─► USB UART      │
                    │                          └─► ESC UART ───► │ ──► Motor Pin
                    │                                             │
  GPIO [2:0] ─────► │  Mux Control (optional external override)  │
                    └─────────────────────────────────────────────┘
```

### SERV Bus Address Map (wb_mux_4)
...
| 0x0300 | DSHOT | Motor control (direct access) |
...
### 3. Clean Bus Separation
- **SPI Bus**: Direct DSHOT access for flight control
- **SERV Bus**: Protocol handling only (no motor access)
- **No Arbiter**: Simplified design, no bus contention
- **GPIO Override**: Optional external mux control
