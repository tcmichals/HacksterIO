...
## Timing Constraints
# Ignore clock feedback output from Clock Wizard (internal PLL signal)
set_false_path -through [get_nets -hierarchical clkfbout_spiDesign_clk_wiz_0_1]

# Async inputs from GPIO - no timing relationship with system clock
set_false_path -from [get_ports {mux_for_esc[*]}]

# Async SPI inputs - treat as false paths (SPI clock domain is separate)
...
# PWM decoder inputs are async from RC receiver
set_false_path -from [get_ports {pwm_ch0 pwm_ch1 pwm_ch2 pwm_ch3 pwm_ch4 pwm_ch5}]

# ESC UART is async from external processor
set_false_path -from [get_ports {esc_uart_tx esc_uart_tx_en}]
set_false_path -to [get_ports esc_uart_rx]

# Motor outputs are registered internally, no critical timing
set_false_path -to [get_ports {motor1 motor2 motor3 motor4}]

# USB UART async
set_false_path -from [get_ports usb_uart_rx]
set_false_path -to [get_ports usb_uart_tx]

# NeoPixel and LEDs are outputs with no critical timing
set_false_path -to [get_ports neopixel]
set_false_path -to [get_ports {led0 led1 led2 led3 debug0 debug1 debug2}]
